...
void doStep(Cavern& cavern) {
  for (int x = 0; x < cavern.len_x; x++) {
    for (int y = 0; y < cavern.len_y; y++) {
      //doesnt matter what order we increase in
      doFlash(cavern, {x, y});
    }
  }
}

bool resetFlashed(Cavern& cavern) {
  int total = 0;
  for (int x = 0; x < cavern.len_x; x++) {
    for (int y = 0; y < cavern.len_y; y++) {
      if (cavern.data[x][y] == '0')
        total++;
      cavern.flashed[x][y] = false;
    }
  }

  return (total == cavern.len_x * cavern.len_y);
}
void solve(std::vector<std::string>& data) {
  ...
  while(++step) {
    ...
    if (resetFlashed(cavern)) {
      std::cout << "Synchronised after " << step << " steps.\n";
      break;
    }
  }

  std::cout << "After " << step << " steps, there were " << cavern.flashes << " flashes.\n";
}
