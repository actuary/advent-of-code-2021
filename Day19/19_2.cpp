...
int taxicabDist(Position posThis, Position posOther) {
  return std::abs(posOther.x - posThis.x)
       + std::abs(posOther.y - posThis.y)
       + std::abs(posOther.z - posThis.z);
}

void solve(std::vector<std::vector<Position>> const& scanners) {
  int const BASE_SCANNER = 12; //NB wouldnt work for all solutions due to this magic number
  ...
  abs_offsets.insert({BASE_SCANNER, 
  ...
  for (int i = 0; i < scanners.size() - 1; i++) {
    for (int j = i + 1; j < scanners.size(); j++) {
      int dist = taxicabDist(abs_offsets.at(i).relative_offset, 
                         abs_offsets.at(j).relative_offset);
